...
`sr_ball(T,N)` - этот предикат находит средний бал по каждому их предметов. Для этого с помощью bagof создадим список с оценками по данному предмету. Далее посчитаем длину этого списка с помощью предиката len, сумму элементов с помощью sum и разделим сумму на количество. Это и будет средний балл.
